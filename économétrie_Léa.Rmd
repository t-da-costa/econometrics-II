...
celec_long <- celec %>%
  pivot_longer(cols = c(Pelec, `IPC(base100=2015)`, Celec_menages), names_to = "Variable", values_to = "Valeur")
```
```{r}
...
```{r}
celec.lm=lm(Celec_menages~PIB2020 + Pop1 + DJU + Pelec + `IPC(base100=2015)` + Date, data = celec)
par(mfrow=c(2,2))
plot(celec.lm)
```
```{r}
